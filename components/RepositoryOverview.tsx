  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      while (true) {
        ...
        if (chunk.includes('# Generated README') || chunk.includes('"# Generated README"')) {
    ...
    <div className="prose prose-slate max-w-none dark:prose-invert px-4">
      <ReactMarkdown
        ...
        components={{
          ...
          h3: ({ node, ...props }) => (
            <h3 className="text-xl font-medium mt-4 mb-2 text-gray-800 dark:text-gray-200" {...props} />
          ),
          p: ({ node, ...props }) => (
            <p className="text-base leading-7 mb-4 text-gray-700 dark:text-gray-300" {...props} />
          ),
          ul: ({ node, ...props }) => (
            <ul className="list-disc list-inside mb-4 ml-4 text-gray-700 dark:text-gray-300" {...props} />
          ),
          li: ({ node, ...props }) => (
          ...
          code: ({ node, inline, ...props }) => (
            ...
            <code className="block bg-gray-100 dark:bg-gray-800 p-4 rounded-lg my-4 text-sm font-mono overflow-auto" {...props} />
          ),
          pre: ({ node, ...props }) => (
            <pre className="bg-gray-100 dark:bg-gray-800 p-4 rounded-lg my-4 overflow-auto" {...props} />
          ),
        }}
      >
        {content}
  ...
  return (
    <Card className="max-w-5xl mx-auto">
      <CardHeader className="border-b">
        <CardTitle className="text-2xl font-bold">Repository: {repoName}</CardTitle>
      </CardHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col space-y-4">
            {!isAnalyzing && !analysisProgress && (
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Would you like to generate a README analysis for this repository?
                This will analyze your code and create a detailed documentation.
              </p>
            )}
